...
async function runDirectFunnel(chatId) {
  // Um único job que dispara foto + vídeo + texto em PARALELO (Promise.all
  // no worker), em vez de 3 jobs separados presos no lock-por-chat (que
  ...
  await queue.add("jobs",
    { type: "SEND_START_SCREEN", chatId: String(chatId), data: {
      photos: ["WhatsApp Image 2026-08-17 at 09.56.27.jpeg"],
      video: "WhatsApp Video 2026-08-17 at 09.56.34.mp4",
      text: START_MESSAGE,
      buttons: [
