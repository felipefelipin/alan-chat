// src/worker2.js — segundo bot/modelo, mesmo fluxo do worker.js. Cópia
// deliberada (ver bot2.js). DB_PREFIX isola os registros desse bot no mesmo
// banco; file_id do Telegram é por bot, nunca reaproveitar o cache do bot1.
...
const { connection, queue } = require("./queue2");
...
const bot = new TelegramBot(process.env.BOT_TOKEN2, { polling: false });
const DB_PREFIX = "m2:";
...
function saveFunnelMsg(chatId, messageId) {
  try {
    prisma.funnelMessage?.create({
      data: { userId: dbId(chatId), messageId: Number(messageId) },
    }).catch(() => {});
  } catch (e) {}
}
...
async function deleteFunnelMsgs(chatId) {
  try {
    const msgs = await prisma.funnelMessage?.findMany({ where: { userId: dbId(chatId) } }) ?? [];
    await Promise.all(msgs.map(m => bot.deleteMessage(chatId, m.messageId).catch(() => {})));
    await prisma.funnelMessage?.deleteMany({ where: { userId: dbId(chatId) } });
  ...
  async (job) => {
    ...
    return withChatLock(chatId, async () => {
      ...
      if (type === "SEND_START_SCREEN") {
        ...
        // Telegram (ver runDirectFunnel em bot2.js).
      ...
      if (type === "SEND_PLANS") {
        await prisma.user.update({ where: { id: dbId(chatId) }, data: { etapa: "checkout" } }).catch(() => {});
...
console.log("worker v3 FINAL (chat-lock) [modelo2] rodando...");