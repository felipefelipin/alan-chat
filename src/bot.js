...
bot.on("callback_query", async (q) => {
  ...
  try {
    ...
    if (data === "chamada_video") {
      ...
      await queue.add("jobs",
        { type: "SEND_MESSAGE", chatId: String(chatId), data: {
          ...
          noTyping: true,
          extra: { parse_mode: "HTML", reply_markup: { inline_keyboard: [
            [{ text: "👉 CLIQUE E VEM ME VER PELADA 😈", web_app: { url: process.env.WEBAPP_URL + "?v=" + Date.now() }, style: "success" }],
          ]}},
        }},
    ...
    if (data.startsWith("plan:")) {
      ...
      prisma.event.create({ data: { userId: String(chatId), type: "PLAN_CLICK", payload: { plano } } }).catch(() => {});
      try {
